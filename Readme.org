...
*** Per-slide transition
  ...
  You can have different transitions on each slide by setting the reveal_data_transition property, like this:
  
: ** Slide Title
:   :PROPERTIES:
:   :reveal_data_transition: default none
:   :END:

  Above, the start transition is set to the default one but the end transition for the slide is set to none.
